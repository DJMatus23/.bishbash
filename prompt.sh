...
cSUCCESS=$LIGHTGREEN # Success block when previous command did return 0
...
cGIT=$LIGHTGRAY
cGITBRANCH=$LIGHTGRAY
cGITMASTER=$GRAY
cGITADD=$GREEN
cGITMOD=$YELLOW
cGITDEL=$RED
cK8S=$LIGHTGRAY
cK8SCONTEXTPROD=$LIGHTRED
cK8SCONTEXTSTAGING=$YELLOW
cK8SCONTEXTOTHER=$GREEN
cK8SNS=$LIGHTBLUE
...
function promptcmd()
{
        ...
        if [ $lGIT -ne 0 ] ; then
                PS1="${PS1}${cLINES}\342\224\214\342\224\200[${cGIT}GIT${cLINES}]\342\224\200"

                lGITBRANCH=$(git rev-parse --abbrev-ref HEAD)
                if [ $lGITBRANCH == "master" ] ; then
                        PS1="${PS1}${cBRACKETS}[${cGITMASTER}${lGITBRANCH}${cBRACKETS}]"
                else
                        PS1="${PS1}${cBRACKETS}[${cGITBRANCH}${lGITBRANCH}${cBRACKETS}]"
                fi

                lGITADD=$(git status -s | grep "^??" | wc -l)
                lGITMOD=$(git status -s | grep "^ M" | wc -l)
                lGITDEL=$(git status -s | grep "^ D" | wc -l)
                if [ $((lGITADD+lGITMOD+lGITDEL)) -ne 0 ] ; then
                        PS1="${PS1}${cLINES}\342\224\200${cBRACKETS}[${cGITMOD}${lGITMOD}/${cGITADD}${lGITADD}/${cGITDEL}${lGITDEL}${cBRACKETS}]"
        ...
        if [ -n "$lK8SCONTEXT" ] ; then
                ...
                PS1="${PS1}${cLINES}[${cK8S}K8S${cLINES}]\342\224\200"

                if [[ $lK8SCONTEXT =~ .*prod.* ]] ; then
                        PS1="${PS1}${cBRACKETS}[${cK8SCONTEXTPROD}${lK8SCONTEXT}${cBRACKETS}]${cLINES}\342\224\200"
                elif  [[ $lK8SCONTEXT =~ .*staging.* ]] ; then
                        PS1="${PS1}${cBRACKETS}[${cK8SCONTEXTSTAGING}${lK8SCONTEXT}${cBRACKETS}]${cLINES}\342\224\200"
                else
                        PS1="${PS1}${cBRACKETS}[${cK8SCONTEXTOTHER}${lK8SCONTEXT}${cBRACKETS}]${cLINES}\342\224\200"
                fi


                lK8SNS="$(kubectl config view --minify --output 'jsonpath={..namespace}' 2>/dev/null)"
                lK8SNS="${lK8SNS:-default}"


                PS1="${PS1}${cBRACKETS}[${cK8SNS}${lK8SNS}${cBRACKETS}]"

                PS1="${PS1}\n"
        fi

        #=========================================================
        # Beginning of third line (arrow wrap around and color setup)
        #=========================================================

        if [ $((lGIT+lK8S)) -ne 0 ] ; then
                PS1="${PS1}${cLINES}\342\224\234\342\224\200"
        else
                PS1="${PS1}${cLINES}\342\224\214\342\224\200"
        fi
